...
uint32_t murmur3_hash(const void *data, const uint32_t &size,
                      const uint32_t &seed) {
  if (!data) return 0;
...
uint64_t murmur3_hash64(const void *str, const uint32_t &size,
                        const uint32_t &seed, const uint32_t &seed2) {
...
std::string md5sum(const void *data, size_t len) {
  MD5_CTX ctx;
  // 将要被弃用，这里忽略此报错 -Wno-deprecated-declarations
  MD5_Init(&ctx);
...
template <class CTX, int (*Init)(CTX *),
...
std::string replace(const std::string &str1, char find,
